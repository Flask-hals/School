...
def make_images():
    ...
    while len(digits) != 10:
        for elements in list:
            values = []
            if elements[1] not in digits:
                digits.append(elements[1])
                for integers in elements[0]:
                    if integers == 0:
                        values.append(255)
                    else:
                        values.append(0)
                png.save_png(28, 28, values, 'digit-{}.png'.format(elements[1]))
...
def predict(model, image):
    ...
    score = float('-inf') # Smallest number possible
    bestDigit = 0
    for digit, percentage in model[0].items():
        tmpscore = 0
        tmpscore += math.log(percentage)
        for index, pixel in enumerate(image):
            if pixel == 1:
                tmpscore += math.log(model[1][digit][index])
            elif pixel == 0:
                tmpscore += math.log(1 - model[1][digit][index])
        if tmpscore > score:
            score = tmpscore
            bestDigit = digit
