...
class LogicCalc
  def initialize
    @logicparser = Parser.new("LogicCalc") do
      @vars = {} # hash containing all set variables
      @setVar = false # boolean that decides if we should set a variable or read
      ...
      token(/and/) {|m| m }
      token(/or/) {|m| m }
      token(/set/) {|m| @setVar=true; m }
      token(/not/) {|m| m }
      token(/[a-zA-Z]+\d*[a-zA-Z]*/) {|m| m}
      token(/\(/) {|m| m }
      token(/\)/) {|m| m }
      ...
      rule :ASSIGN do
        match("(", "set", :VAR, :EXPR, ")") {|_,_,a,b,_| 
          if a == 'true' || a == 'false'
            "Not allowed variable name! Can't be 'true' or 'false'"
          else
            @vars[a] = b
          end}
      end
