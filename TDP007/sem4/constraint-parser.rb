...
class ConstraintParser < Parser
  ...
  def initialize
    @connectors={}
    @subtractor = false # Bool to decide output if Adder is created for subtraction.
    @parser=super("constraints") do
      ...
      rule :atom do
        match(Connector)
        match('(', :expr, ')') {|_, a, _| 
                                  if @subtractor && a.is_a?(ArithmeticConstraint)
                                    @subtractor = false
                                    a.b; 
                                  elsif a.is_a?(ArithmeticConstraint)
                                    a.out 
                                  else
                                    a 
                                  end;}
  ...
  #New function
  #Returns output from a operator
  def get_connector(arObj)
    if @subtractor && arObj.is_a?(ArithmeticConstraint)
      @subtractor = false
      arObj.b; 
    elsif arObj.is_a?(ArithmeticConstraint)
      arObj.out 
    else
      arObj 
    end;
...
#c,f=cp.parse "10+x=2*(y+2)"
# f, c = cp.parse "5+f=4+c"
