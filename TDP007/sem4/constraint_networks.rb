...
class ArithmeticConstraint
  ...
  def new_value(connector)
    #New code
    #Checks which value that's not defined
    if [a,b,out].include?(connector)
      if a.has_value? and b.has_value? and 
...
class CtooF
  attr_accessor :c, :f
  def initialize(c, f)
    nineMult = ConstantConnector.new("9", 9) #Värde vi vill multiplicera
    fiveDiv = ConstantConnector.new("5", 5) #Värde vi vill dividera
    add = ConstantConnector.new("32", 32) #Värde vi vill subtrahera
    c9 = Connector.new("9c")
    f32 = Connector.new("f-32")
    
    Multiplier.new(c, nineMult, c9) # Räknar ut C * 9
    Multiplier.new(f32, fiveDiv, c9)  # Räknar ut (C * 9) / 5 = (f - 32)
    Adder.new(f32, add, f) # Räknar ut (f - 32) + 32 = f
  end
end
